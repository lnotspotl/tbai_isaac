...
class CentralPatternGenerator:
    ...
    def leg_heights(self, phase_offsets=None, phases=None):
        ...
        heights[swing_up_indeces] = 0.1 * (-2 * torch.pow(time_up, 3) + 3 * torch.pow(time_up, 2))
        ...
        heights[swing_down_indeces] = 0.1 * (2 * torch.pow(time_down, 3) - 3 * torch.pow(time_down, 2) + 1.0)
