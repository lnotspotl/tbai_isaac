...
import numpy as np
import tbai_isaac.envs.go2.perceptive.config as ac
import torch
from isaacgym import gymapi, gymtorch, gymutil
from isaacgym.torch_utils import *
from tbai_isaac.envs.go2.common.central_pattern_generator import CentralPatternGenerator
from tbai_isaac.envs.go2.common.inverse_kinematics import Go2InverseKinematics
from tbai_isaac.envs.go2.info import URDF_FOLDER
...
class LeggedRobot(BaseEnv):
    def __init__(self, yaml_cfg, headless):
        ...
        self.asset_config = ac.get_asset_config(self.config)
        self.domain_randomization_config = ac.get_randomization_config(self.config)
        self.terrain_config = ac.get_terrain_config(self.config)
        self.command_config = ac.get_command_config(self.config)
        self.env_config = ac.get_env_config(self.config)
        self.control_config = ac.get_control_config(self.config)
        self.viewer_config = ac.get_viewer_config(self.config)
        self.normalization_config = ac.get_normalization_config(self.config)
        self.rewards_config = ac.get_rewards_config(self.config)
        self.init_state_config = ac.get_init_state_config(self.config)
        self.noise_config = ac.get_noise_config(self.config)
        self.sim_config = ac.get_sim_config(self.config)

        self.observation_managers: OrderedDict[str, ObservationManager] = OrderedDict()

        self.clip_actions = select(self.config, "environment.normalization.clip_actions")

        self.sim_params = ac.get_sim_params(self.config)
        ...
        assert self.asset_config.name == "go2", f"Unknown robot name: {self.asset_config.name}"
        self.aik = Go2InverseKinematics(device=self.device)
    ...
    def update_command_curriculum(self, env_ids):
        ...
        if (
            ...
            self.command_ranges.lin_vel_x[0] = float(np.clip(
                self.command_ranges.lin_vel_x[0] - 0.05, -self.command_config.max_curriculum, 0.0
            ))
            self.command_ranges.lin_vel_x[1] = float(np.clip(
                self.command_ranges.lin_vel_x[1] + 0.05, 0.0, self.command_config.max_curriculum
            ))
    ...
    def _init_buffers(self):
        ...
        if "go2" in self.asset_config.name:
            ...
            raise NameError(f"Unknown robot type: {self.asset_config.name}. Robot should be named 'go2'.")
        ...
        for i in range(self.num_dofs):
            ...
            angle = self.config.environment.init_state.default_joint_angles[name]
    ...
    def _create_envs(self):
        ...
        asset_options = ac.get_asset_options(self.config)
    ...
    def _parse_cfg(self):
        ...
        self.reward_scales = self.config.environment.rewards.scales
