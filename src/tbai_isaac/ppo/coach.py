...
class Coach:
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar("Iteration", locs["it"], locs["it"])
        ...
        if len(locs["rewbuffer"]) > 0:
            log_string = (
                ...
                f"""{'Mean reward:':>{pad}} {statistics.mean(locs['rewbuffer']):.4f}\n"""
                f"""{'Mean episode length:':>{pad}} {statistics.mean(locs['lenbuffer']):.4f}\n"""
            )
        #   f"""{'Mean reward/step:':>{pad}} {locs['mean_reward']:.4f}\n"""
        #   f"""{'Mean episode length/episode:':>{pad}} {locs['mean_trajectory_length']:.4f}\n""")
        else:
            log_string = (
                f"""{'#' * width}\n"""
                f"""{str.center(width, ' ')}\n\n"""
        ...
        log_string += (
            ...
            f"""{'Iteration time:':>{pad}} {iteration_time:.4f}s\n"""
            f"""{'Total time:':>{pad}} {self.tot_time:.4f}s\n"""
